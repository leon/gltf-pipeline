...
Remove.image = function (gltf, imageId) {
  ...
  ForEach.texture(gltf, function (texture) {
    ...
    if (
      ...
      defined(ext) &&
      defined(ext.EXT_texture_avif) &&
      ext.EXT_texture_avif.source > imageId
    ) {
...
getListOfElementsIdsInUse.image = function (gltf) {
  ...
  ForEach.texture(gltf, function (texture) {
    ...
    if (
      ...
      defined(texture.extensions) &&
      defined(texture.extensions.EXT_texture_avif)
    ) {
      usedImageIds[texture.extensions.EXT_texture_avif.source] = true;
    } else if (
      defined(texture.extensions) &&
